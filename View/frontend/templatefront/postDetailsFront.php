...
<?php
    ...
    require '../../../controller/CommentssController.php';
    ...
    $commentController = new CommenttController();
    ...
    $postUser = $postController->getPostWithUser($postId);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if (isset($_POST['comment'])) {
          ...
          if (!empty($comment)) {
              $commentController->create($postId, $currentUserId, $comment);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (isset($postData['action']) && $postData['action'] === 'vote') {
      ...
      try {
          $existingVote = $likeDislikeModel->hasVoted($currentUserId, $postId, $commentId);
          if ($existingVote) {
              if ($existingVote['is_like'] == $isLike) {
                  $likeDislikeController->cancelVote($currentUserId, $postId, $commentId);
                  echo json_encode(["success" => true, "message" => "Vote cancelled."]);
              } else {
                  $likeDislikeController->updateVote($currentUserId, $postId, $commentId, $isLike);
                  echo json_encode(["success" => true, "message" => "Vote updated."]);
              }
          } else {
              $likeDislikeController->vote($currentUserId, $postId, $commentId, $isLike);
                ...
                <?php if ($postData): ?>
                    ...
                    <div class="details-section">
                        ...
                        <div class="author">
                            ...
                            <div>
                    <h6>Posted by: <?php echo htmlspecialchars($postUser->username); ?></h6>
                    <small><?php echo htmlspecialchars($postData->getCreatedAt()); ?></small>
                </div>
                        </div>
                        ...
                        <div class="comments-section">
                            ...
                            <?php if (!empty($comments)): ?>
                                <?php foreach ($comments as $comment):
                                    $commentLiked = $likeDislikeModel->isCommentLikedByUser($currentUserId, $comment->getId());
                                    $commentDisliked = $likeDislikeModel->isCommentDislikedByUser($currentUserId, $comment->getId());
