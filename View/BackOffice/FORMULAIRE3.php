...
<body>
    ...
    $current_question = isset($_GET['i']) ? (int)$_GET['i'] : 1;

    // Si toutes les questions ont été soumises
    if ($current_question > $num_questions) {
        header("Location: listeQuiz.php");
    }

    // Afficher le formulaire pour la question actuelle
    echo "<form id='questionForm_$current_question' align='center' action='ADDquestion+reponses1.php?quiz_id=" . $quiz_id . "&num_questions=" . $num_questions . "&i=" . $current_question . "' method='post' onsubmit='return validateForm($current_question);'>";

    
    echo "<label for='question'>Question $current_question :</label>";
