...
class Post {
    ...
    public function create() {
        $stmt = $this->pdo->prepare("INSERT INTO posts (title, content, video_name, created_at, user_id) VALUES (:title, :content, :video_name, :created_at, :user_id)");
        return $stmt->execute([
            'title' => $this->title,
    ...
    public function getById($id, $user_id = null) {
        $stmt = $this->pdo->prepare("SELECT * FROM posts WHERE id = :id AND (user_id = :user_id OR :user_id IS NULL)");
    ...
    public function update() {
        $stmt = $this->pdo->prepare("UPDATE posts SET title = :title, content = :content, video_name = :video_name WHERE id = :id AND user_id = :user_id");
        return $stmt->execute([
            'title' => $this->title,
            'content' => $this->content,
            'video_name' => $this->video_name,
